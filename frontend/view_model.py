...
class BluetoothDevice(MenuPage):
    def __init__(self, device, bluetooth_page):
        super().__init__(device["name"], bluetooth_page, has_sub_page=False)
        self.device = device
    ...
    def render(self):
        # Set the line type based on whether the device is connected or not
        line_type = (
            LINE_HIGHLIGHT
            if self.device == self.parent_page.connected_device
            else LINE_NORMAL
        )
        ...
        return f"{connected_indicator}{self.device['name']}", line_type
