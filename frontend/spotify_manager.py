...
def get_playlist(id):
    ...
    return (
        ...
        tracks,
    )  # return playlist index as 0 because it won't have a idx parameter when fetching directly from Spotify (and we don't need it here anyway)
...
def get_show(id):
    ...
    return (
        UserShow(results["name"], publisher, len(episodes), results["uri"]),
        episodes,
    )
...
def play_from_playlist(playist_uri, track_uri, device_id=None):
    ...
    if not device_id:
        ...
        device_id = devices[0].id
    sp.start_playback(
        device_id=device_id, context_uri=playist_uri, offset={"uri": track_uri}
    )
    refresh_now_playing()
...
def play_from_show(show_uri, episode_uri, device_id=None):
    ...
    if not device_id:
        ...
        device_id = devices[0].id
    sp.start_playback(
        device_id=device_id, context_uri=show_uri, offset={"uri": episode_uri}
    )
    refresh_now_playing()
...
def get_now_playing_track(response=None):
    ...
    if context["type"] == "playlist":
        ...
        now_playing["track_index"] = (
            next(x for x, val in enumerate(tracks) if val.uri == track_uri) + 1
        )
