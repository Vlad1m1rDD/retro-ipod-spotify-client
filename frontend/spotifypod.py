...
class Marquee(tk.Canvas):
    ...
    def set_text(self, text):
        ...
        if self.width > self.winfo_width():
            ...
            self.coords(
                "text",
                (self.winfo_width() / 2) - (self.width / 2),
                self.winfo_height() / 2,
            )
        self.pause_ctr = 100
        self.reset = True
    ...
    def redraw(self):
        ...
        if win_width < 2:
            ...
            self.coords(
                "text", (win_width / 2) - (self.width / 2), self.winfo_height() / 2
            )
            return
...
class SearchFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        divider.grid(
            row=1, column=0, sticky="we", pady=(10, int(160 * SCALE)), padx=(10, 30)
        )
        contentFrame = tk.Canvas(
            self, bg=SPOT_BLACK, highlightthickness=0, relief="ridge"
        )
        ...
        search_line.configure(
            bg=SPOT_GREEN, height=5, bd=0, highlightthickness=0, relief="ridge"
        )
        search_line.grid(row=3, column=0, sticky="we", pady=10, padx=120)
        self.loading_label = tk.Label(
            self, text="", font=LARGEFONT, background=SPOT_BLACK, foreground=SPOT_WHITE
        )
        self.loading_label.grid(
            row=4, column=0, sticky="we", pady=(int(100 * SCALE), 0)
        )
...
class NowPlayingFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.progress_frame = tk.Canvas(
            contentFrame, height=int(72 * SCALE), bg=SPOT_BLACK, highlightthickness=0
        )
        self.progress_frame.grid(
            row=4, column=0, sticky="we", pady=(int(52 * SCALE), 0), padx=(30, 50)
        )
    ...
    def update_now_playing(self, now_playing):
        if not self.inflated:
            ...
            if parent_width > 2:
                ...
                self.progress = self.progress_frame.create_rectangle(
                    self.progress_start_x,
                    0,
                    self.midpoint,
                    int(72 * SCALE),
                    fill=SPOT_GREEN,
                )
                self.progress_frame.create_image(
                    self.midpoint,
                    (self.frame_img.height() - 1) / 2,
        ...
        truncd_context = (
            truncd_context if len(truncd_context) < 20 else truncd_context[0:17] + "..."
        )
        self.header_label.configure(text=truncd_context)
        update_delta = (
            0
            if not now_playing["is_playing"]
            else (time.time() - now_playing["timestamp"]) * 1000.0
        )
        ...
        if self.update_time:
            progress_txt = ":".join(
                str(timedelta(milliseconds=adjusted_progress_ms))
                .split(".")[0]
                .split(":")[1:3]
            )
            remaining_txt = "-" + ":".join(
                str(timedelta(milliseconds=adjusted_remaining_ms))
                .split(".")[0]
                .split(":")[1:3]
            )
        ...
        if self.inflated:
            adjusted_progress_pct = min(
                1.0, adjusted_progress_ms / now_playing["duration"]
            )
            self.progress_frame.coords(
                self.progress,
                self.progress_start_x,
                0,
        ...
        if now_playing["track_index"] < 0:
            ...
            return
        context_str = (
            str(now_playing["track_index"]) + " of " + str(now_playing["track_total"])
        )
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.green_arrow_image = ImageTk.PhotoImage(
            flattenAlpha(Image.open("pod_arrow_grn.png"))
        )
        self.black_arrow_image = ImageTk.PhotoImage(
            flattenAlpha(Image.open("pod_arrow_blk.png"))
        )
        self.empty_arrow_image = ImageTk.PhotoImage(
            flattenAlpha(Image.open("pod_arrow_empty.png"))
        )
        ...
        self.play_indicator = tk.Label(
            header_container, image=self.space_image, background=SPOT_BLACK
        )
        self.play_indicator.grid(sticky="w", column=0, row=0, padx=(70 * SCALE, 0))
        self.wifi_indicator = tk.Label(
            header_container, image=self.space_image, background=SPOT_BLACK
        )
        ...
        self.scrollBar = tk.Canvas(
            self.scrollFrame, bg=SPOT_GREEN, highlightthickness=0, width=int(20 * SCALE)
        )
        self.scrollBar.place(
            in_=self.scrollFrame,
            relx=0.5,
            y=int(10 * SCALE),
            anchor="n",
            relwidth=0.6,
        ...
        for x in range(6):
            ...
            imgLabel = tk.Label(
                listFrame, image=self.green_arrow_image, background=SPOT_BLACK
            )
    ...
    def set_header(self, header, now_playing=None, has_wifi=False):
        ...
        if now_playing is not None:
            play_image = (
                self.play_image if now_playing["is_playing"] else self.pause_image
            )
    ...
    def set_list_item(self, index, text, line_type=LINE_NORMAL, show_arrow=False):
        ...
        txtColor = (
            ...
            else (SPOT_GREEN if line_type == LINE_NORMAL else SPOT_WHITE)
        )
        truncd_text = text if len(text) < 17 else text[0:15] + "..."
        self.listItems[index].configure(
            background=bgColor, foreground=txtColor, text=truncd_text
        )
        arrow = self.arrows[index]
        arrow.grid(row=index, column=1, sticky="nsw", padx=(0, 30))
        arrowImg = (
            self.empty_arrow_image
            if not show_arrow
            else (
                self.black_arrow_image
                if line_type == LINE_HIGHLIGHT
                else self.green_arrow_image
            )
        )
...
def render_menu(app, menu_render):
    ...
    for i, line in enumerate(menu_render.lines):
        page.set_list_item(
            i, text=line.title, line_type=line.line_type, show_arrow=line.show_arrow
        )
    page.set_header(
        menu_render.header, menu_render.now_playing, menu_render.has_internet
    )
