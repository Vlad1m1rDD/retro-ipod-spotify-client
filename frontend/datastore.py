...
class Datastore:
    ...
    def setBluetoothDevice(self, device):
        ...
        key = f"bluetooth_device:{device['addr']}"
        self.r.set(key, pickle.dumps(device))

    def getSavedBluetoothDevice(self, addr):
        key = f"bluetooth_device:{addr}"
        pickled_device = self._getSavedItem(key)
        if pickled_device:
    ...
    def getAllSavedPlaylists(self):
        return list(
            map(lambda idx: self._getSavedItem(idx), self.r.keys("playlist-uri:*"))
        )

    def getAllSavedAlbums(self):
        return list(
            map(lambda idx: self._getSavedItem(idx), self.r.keys("album-uri:*"))
        )
